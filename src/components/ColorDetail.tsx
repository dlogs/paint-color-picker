...
const StatBlock = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <div>
    <p className="opacity-50 uppercase text-[10px] sm:text-xs tracking-widest sm:tracking-[0.25em] font-black mb-2 sm:mb-3">
      {label}
    </p>
...
const ColorDetail = ({ allColors, onAccentChange }: ColorDetailProps) => {
  ...
  return (
    <div
      className={`w-full max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24 ${textColor}`}
    >
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <AddToPaletteDialog swatch={color} hasAccent={true} isDarkAccent={isDark} />
        </div>
      </div>
      ...
      <div className="w-full max-w-2xl mb-12 sm:mb-20 pt-4 sm:pt-8 relative z-10">
        <h1 className="text-4xl sm:text-6xl font-black mb-3 sm:mb-4 tracking-tighter flex flex-wrap items-center gap-3">
          ...
          {color.number && (
            <span className="opacity-50 font-medium text-2xl sm:text-4xl whitespace-nowrap">
              ({color.number})
            </span>
          )}
        </h1>
        ...
        <div className={`grid grid-cols-3 gap-4 sm:gap-10 pt-8 sm:pt-10 border-t ${borderColor}`}>
          <StatBlock label="Lightness">{Math.round(L * 100)}%</StatBlock>
          <StatBlock label="Chroma">{C.toFixed(3)}</StatBlock>
          <StatBlock label="Hue">
            <HuePill
              hue={H}
      ...
      <div className="space-y-12">
        ...
        <div className="relative overflow-hidden pt-8">
          ...
          {matches.length > 0 ? (
            ...
            !isSearching && (
              <div
                className={`py-24 text-center border-2 border-dashed rounded-3xl ${borderColor}`}
              >
