...
export function HueRangeSlider({
    ...
    useEffect(() => {
        ...
        const handleTouchMove = (e: TouchEvent) => {
            ...
            const newValue = [...value] as [number, number];
            newValue[activeThumb] = Math.round(calculateAngle(e.touches[0].clientX, e.touches[0].clientY));
            onValueChange(newValue);
        };
    ...
    const thumbStyle = { filter: "drop-shadow(0 0 4px rgba(0,0,0,0.8))", transformBox: "fill-box" as const, transformOrigin: "center" };
    ...
    return (
        <div className={cn("relative flex items-center justify-center select-none", className)} style={{ width: size, height: size }}>
            ...
            <div className="absolute z-50 flex items-center justify-center pointer-events-none" style={{ width: size, height: size }}>
                <div
                    className={cn(
                ...
                >
                    {isFullRange ? (
                        <div className="text-[9px] leading-tight opacity-50 uppercase tracking-tighter">All<br />Colors</div>
                    ) : (
                ...
                {!isFullRange && (
                    <g className="cursor-pointer">
                        <circle
                            ...
                            stroke="white"
                            strokeWidth={2}
                            className={cn("shadow-2xl transition-transform", activeThumb === 0 ? "scale-125" : "hover:scale-110")}
                            style={thumbStyle}
                        />
                        <circle
                            cx={thumb1.x}
                            ...
                            stroke="white"
                            strokeWidth={2}
                            className={cn("shadow-2xl transition-transform", activeThumb === 1 ? "scale-125" : "hover:scale-110")}
                            style={thumbStyle}
                        />
                    </g>
                )}
